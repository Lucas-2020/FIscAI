...
export default function NFSeResultView() {
    const { user } = useAuth();
    ...
    const handleGerarSugestao = async (dadosFormulario) => {
        ...
        try {
            if (USAR_CACHE_BANCO) {
                const respBanco = await api.post('/bigdata/nfse/mostra/entrada', dadosFormulario);
        ...
        try {
            const resp = await api.post('/llms/nfse', dadosFormulario);
            ...
            if (resp.data?.erro_orientacao) {
                ...
                try {
                    await api.post('/bigdata/nfse/cria', dadosSalvar);
                } catch (saveError) { }
            }
        } catch (err) {
                            ...
                            background:
                                theme =>
                                    theme.palette.mode === "dark"
                                        ? "linear-gradient(135deg, rgba(156,39,176,.16), rgba(0,0,0,0))"
                                        : "linear-gradient(135deg, rgba(156,39,176,.10), rgba(255,255,255,0))",
                        }}
                    >
                        <Box sx={{ fontWeight: 800, fontSize: 16 }}>Entrada • NFS-e</Box>
                        <Box sx={{ mt: 0.5, color: "text.secondary", fontSize: 13 }}>
                            Informe UF/Município e dados do serviço para gerar a sugestão.
                        </Box>
                    </Box>

                    <Box sx={{ p: { xs: 2, sm: 2.5 } }}>
                        <FormNFSe ref={formRef} onGerarSugestao={handleGerarSugestao} disabled={loading} />
                    </Box>
                </Paper>
            </Box>
